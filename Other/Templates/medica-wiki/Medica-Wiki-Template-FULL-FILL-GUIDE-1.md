...
4. Potency gauge, effects, compounds.
5. **"View Product →" button** — a clickable link to the item's page.
6. Receptor table (unchanged from before).
...
{ name:"Common Name", latin:"Scientific name",
  ...
  tags:["Tag1","Tag2"],
  link:"https://example.com/products/common-name",
  potency:0.0,
...
| `link` | string (URL) | Powers the amber **"View Product →"** button on the card. Opens in a new tab (`target="_blank"`). Should be a direct URL to the item's page — not a search page or homepage. |
...
- [ ] `link` is a direct, working URL to the item's page (not blank, not a placeholder domain in production).
